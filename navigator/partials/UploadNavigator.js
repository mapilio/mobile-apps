...
const UploadNavigator = () => {
  ...
  const sequenceListener = ({ navigation, route }) => ({
    ...
    beforeRemove: (e) => {
      navigation
        .getParent()
        .setOptions({ tabBarStyle: { display: 'flex', height: tabHeight + bottom } });
    },
  });
  ...
  return (
    <Stack.Navigator
      ...
      <Stack.Screen
        ...
        options={{
          headerLeft: (props) => (
            <SequenceNavigatorLeft {...props} route backRoute={Routes.sequences} />
          ),
      ...
      <Stack.Group
        screenOptions={{ presentation: 'card', gestureEnabled: false, headerShown: false }}>
