...
const ActiveImage = ({
  ...
  const report = (reason) => {
    api
      ...
      .then(() => {
        isFullScreen
          ? showToast(t('report_success'), { type: 'success', hideToast })
          : toast.show(t('report_success'), { type: 'success' });
      })
      .catch(() => {
        isFullScreen
          ? showToast(t('report_error'), { type: 'error', hideToast })
    ...
    <View style={[styles.imageWrapper, fullScreenBorder]}>
      <ImageBackground
        ...
        style={[
          ...
          !isFullScreen && { borderRadius: RFValue(10) },
        ]}
        ...
        <View style={[{ flex: 1 }, isFullScreen && { marginHorizontal: left }]}>
          ...
          <View style={styles.infoArea}>
            <CustomTextBold style={styles.h1}>
              {sequenceName ? maxCharacterHandler(sequenceName, 30) : t('no_address')}
            </CustomTextBold>
