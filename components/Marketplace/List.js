...
const Detail = ({ project, onClose, setOnScroll, navigation }) => {
  ...
  const {
    properties: { id, owner, marketplace_name, marketplace_description, project_camera_type },
    geometry,
  } = project;
      ...
      isNear(targetPoint)
        .then((distance) => {
          if (distance > 5) {
            ...
            api
              .post(`/api/function/projects/job/createJob`, { options: { parameters: { id: id } } })
              .then(() => {
  ...
  return (
    <View style={marketplaceStyles.container}>
      ...
      <View style={marketplaceStyles.listHeader} onTouchStart={() => setOnScroll(false)}>
        ...
        />
        <CustomText style={marketplaceStyles.title}>{t('marketplace')}</CustomText>
        <MarketplacePopover />
      </View>
      <ScrollView
