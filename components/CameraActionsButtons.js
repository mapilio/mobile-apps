...
const CameraActionsButtons = () => {
  ...
  const { GPSAccuracy, GPSStartAccuracy, batteryLevel, mocked, accuracy, batteryStatus } =
  ...
  useEffect(() => {
    if (!waitGPS) {
      dispatch({
        type: CAPTURE_BUTTON_STATUS,
        payload: GPSAccuracy || !batteryStatus || !mocked || accuracy.isTrue,
      });
    }
