...
const Pano = ({ pointInformation, hidePano }) => {
  ...
  useEffect(() => {
    api
      .get(`/api/search-user?options[parameters][id]=${pointInformation.user}`)
      .then((res) => {
  ...
  return (
    <View style={{ zIndex: 10 }}>
      ...
      <View style={panoStyle.info}>
        <View style={panoStyle.capturer}>
          <TouchableOpacity
            ...
            <Text style={panoStyle.capturer.name}>
              {userDetails && maxCharacterHandler(userDetails.username, 12)}
            </Text>
          </TouchableOpacity>
      ...
      <LinearGradient colors={['#11111100', '#111111']} angle={180} style={panoStyle.bottomTab}>
