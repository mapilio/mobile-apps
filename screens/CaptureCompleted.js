...
const CaptureCompleted = () => {
  ...
  const calculateTime = async () => {
    ...
    const firstTime = new Date(
      dateConvert(
        exif1.DateTime ||
          exif1.DateTimeOriginal ||
          exif1.DateTimeDigitized ||
          exif1['{TIFF}'].DateTime,
        'MMM D, YYYY HH:mm:ss'
      )
    );
    const lastTime = new Date(
      dateConvert(
        exif2.DateTime ||
          exif2.DateTimeOriginal ||
          exif2.DateTimeDigitized ||
          exif2['{TIFF}'].DateTime,
        'MMM D, YYYY HH:mm:ss'
      )
    );
  ...
  const checkData = async () => {
    ...
    const line = lineString(
      data.map((item) => [JSON.parse(item.location).longitude, JSON.parse(item.location).latitude])
    );
    const point = points(
      data.map((item) => [JSON.parse(item.location).longitude, JSON.parse(item.location).latitude])
    );
  ...
  useEffect(() => {
    ...
    return () =>
      navigation
        .getParent()
        .setOptions({ tabBarStyle: { display: 'flex', height: RFValue(63) + bottom } });
  }, []);
  ...
  const MapLoader = () => (
    <View style={{ ...styles.map, ...styles.mapLoader }}>
