...
const UserSequence = ({ navigation }) => {
  ...
  const getData = async () => {
    try {
      ...
      const lines = Object.keys(splitSequences).reduce((acc, key) => {
        acc[key] = lineString(
          splitSequences[key].map(({ location }) => [
      ...
      const coordinates = result.map(({ location }) => [
        JSON.parse(location).longitude,
        JSON.parse(location).latitude,
      ]);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={goBack} style={{ ...styles.backButton, top: top + RFValue(20) }}>
