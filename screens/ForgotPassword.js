  ...
  return (
    <View style={loginStyles.container}>
      ...
      <View style={{ marginBottom: RFValue(30) }}>
        <CustomTextBold style={loginStyles.primaryText}>{t('title')}</CustomTextBold>
        <CustomText style={loginStyles.secondaryText}>{t('description')}</CustomText>
      </View>
      ...
      <Controller
        ...
        render={({ field: { onChange, onBlur, value } }) => (
          <View style={{ ...loginStyles.formGroup }}>
            {errors.email && (
              <Text style={loginStyles.errorText}>{t(errors.email.message, { ns: 'form' })}</Text>
            )}
            <TextInput
              name="email"
              placeholder={t('email')}
      ...
      <TouchableOpacity
        ...
        onPress={handleSubmit((values) => forgotPassword(values))}>
