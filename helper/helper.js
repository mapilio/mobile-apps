...
const fetchHandler = ({ ...args } = {}) => {
  ...
  auth &&
    (axios.defaults.headers.common['Authorization'] = `Bearer ${auth.access_token || auth.token}`);
...
const dateConvert = (datetime, format = 'MMM D, YYYY') => {
  if (!Moment(datetime).isValid()) {
    ...
    return Moment(new Date(parsedDatetime.join(' ')))
 ...
 */
const getContentAreaHeight = (top, bottom) =>
  Dimensions.get('window').height - tabHeight - top - bottom;
