...
export const getHash = async (image) => {
  ...
  try {
    ...
    await db.runAsync('UPDATE captures SET uploaded=1, hash=? WHERE id=?', [
      response.files[0].hash,
      image.id,
    ]);
...
export const imageryUpload = async (images, sequence_uuid) => {
  ...
  for (let image of images) {
    ...
    try {
      ...
      const horizontal_fov = calculate.fov(
        horizontal_pixel,
        vertical_pixel,
        exif.FocalLength,
        'horizontal'
      );
      const vertical_fov = calculate.fov(
        horizontal_pixel,
        vertical_pixel,
        exif.FocalLength,
        'vertical'
  ...
  try {
    const response = await api.post('/api/function/mapilio/imagery/upload', files, {
      cancelToken: apiController?.token,
    });
