...
const renderMeasured = (element, layout = { width: 120, height: 24 }) => {
  ...
  const measuredContainer = tree.root.find(
...
describe("SkeletonPlaceholder", () => {
  ...
  it("returns children unchanged when disabled", () => {
    ...
    act(() => {
      tree = renderer.create(
        <SkeletonPlaceholder enabled={false}>{child}</SkeletonPlaceholder>
      );
    });

    expect(tree.root.findByProps({ testID: "content" }).props).toEqual(
      child.props
    );
  });
  ...
  it("clips each leaf shimmer to the leaf bounds", () => {
    ...
    const leaf = tree.root
      ...
      .find(
        (node) =>
          StyleSheet.flatten(node.props.style)?.backgroundColor === "#E1E9EE"
      );
  ...
  it("stops the animation loop when animation becomes disabled", () => {
    ...
    const tree = renderMeasured(
      <SkeletonPlaceholder>{child}</SkeletonPlaceholder>
    );

    act(() => {
      tree.update(
        <SkeletonPlaceholder speed={0}>{child}</SkeletonPlaceholder>
      );
    });
