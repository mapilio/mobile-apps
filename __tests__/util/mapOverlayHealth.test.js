...
describe("probeVectorTile", () => {
  ...
  it("uses a bounded HEAD request and reports success", async () => {
    ...
    await expect(
      probeVectorTile({...request, fetchImplementation})
    ).resolves.toEqual({available: true, reason: null, status: 200});
  ...
  it("aborts a probe that exceeds its time limit", async () => {
    jest.useFakeTimers();
    const fetchImplementation = jest.fn((url, options) =>
